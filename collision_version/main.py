...
score = Score(SCORE_POSITION,
            GAME_OVER_MESSAGE_POSITION,
            CLEAR_MESSAGE_POSITION
            )
...
def main() :
    ...
    window.onkey(score.clear_max_score, "k")

    while True:
        #If the snake collides the walls, the game is over
        if  (head.xcor() >= WIDTH or head.xcor() <= -WIDTH
                or head.ycor() >= HEIGHT or head.ycor() <= -HEIGHT):
            score.game_over()
            break
        snake.move()
